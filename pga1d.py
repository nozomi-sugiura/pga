...
# gd = np.zeros_like(g)
# for jp in range(npath):
#     gd[:,jp] = si.prodsig(g0_i,g[:,jp],d,n)
...
#     tt[jp] = np.sum(v1[:]*si.logsig(gd,d,n))
 ...
 for j in range(ntr):
   ...
   norm_old = 1e20
   for i in range(ntr2):
     v1_new, rho, eps2 = pg.nesterov_p(v1,v1_old,rho,eps2,tt,g,g0,\
                                       npath,cov,d,n,nnp,P,P_i,cov_i)
     v1_old  = v1.copy()
     v1      = v1_new.copy()
   if (j+1)%10 == 0:
     t1  = np.mean(tt**2)
     ...
     fname1 = 'lm_n=4_j=' + str(j)
     np.savez(fname1, gm, lp0,ln0, lp1, ln1)
     fname2 = 'PGA1_n=4_j=' + str(j)
     np.savez(fname2, tt, v1)
